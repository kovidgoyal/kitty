...
create_os_window(PyObject UNUSED *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "OOsss|Oiipz", &get_window_size, &pre_show_callback, &title, &wm_class_name, &wm_class_class, &load_programs, &x, &y, &background, &background_monitor)) return NULL;
