...
case offset_from_parent_y: value_state = INT; break;
            ...
            case EQUAL:
                if (parser_buf[pos++] != '=') {
                    REPORT_ERROR("Malformed GraphicsCommand control block, no = after key, found: 0x%x instead", parser_buf[pos-1]);
                    return;
                }
                state = value_state;
                break;
            ...
            case delete_action: {
                ...
                if (g.delete_action != 'A' && g.delete_action != 'C' && g.delete_action != 'F' && g.delete_action != 'I' && g.delete_action != 'N' && g.delete_action != 'P' && g.delete_action != 'Q' && g.delete_action != 'R' && g.delete_action != 'X' && g.delete_action != 'Y' && g.delete_action != 'Z' && g.delete_action != 'a' && g.delete_action != 'c' && g.delete_action != 'f' && g.delete_action != 'i' && g.delete_action != 'n' && g.delete_action != 'p' && g.delete_action != 'q' && g.delete_action != 'r' && g.delete_action != 'x' && g.delete_action != 'y' && g.delete_action != 'z') {
            ...
            case transmission_type: {
                ...
                if (g.transmission_type != 'd' && g.transmission_type != 'f' && g.transmission_type != 's' && g.transmission_type != 't') {
...
#define I(x) case x: g.x = is_negative ? 0 - (int32_t)code : (int32_t)code; break
                READ_UINT;
                switch(key) {
                    I(z_index); I(offset_from_parent_x); I(offset_from_parent_y);
                    default: break;
                }
                state = AFTER_VALUE;
                break;
...
#define U(x) case x: g.x = code; break
                switch(key) {
                    U(format); U(more); U(id); U(image_number); U(placement_id); U(quiet); U(width); U(height); U(x_offset); U(y_offset); U(data_height); U(data_width); U(data_sz); U(data_offset); U(num_cells); U(num_lines); U(cell_x_offset); U(cell_y_offset); U(cursor_movement); U(unicode_placement); U(parent_id); U(parent_placement_id); U(usage_hints);
                    default: break;
                }
                state = AFTER_VALUE;
                break;
...
#undef READ_UINT
            ...
            case AFTER_VALUE:
                switch (parser_buf[pos++]) {
                    default:
                        REPORT_ERROR("Malformed GraphicsCommand control block, expecting a , or semi-colon after a value, found: 0x%x",
                                     parser_buf[pos - 1]);
                        return;
                    case ',':
                        state = KEY;
