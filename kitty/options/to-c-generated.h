...
static void
convert_from_python_allow_resize_csi(PyObject *val, Options *opts) {
    opts->allow_resize_csi = PyObject_IsTrue(val);
}

static void
convert_from_opts_allow_resize_csi(PyObject *py_opts, Options *opts) {
    PyObject *ret = PyObject_GetAttrString(py_opts, "allow_resize_csi");
    if (ret == NULL) return;
    convert_from_python_allow_resize_csi(ret, opts);
    Py_DECREF(ret);
}
...
convert_opts_from_python_opts(PyObject *py_opts, Options *opts) {
    ...
    if (PyErr_Occurred()) return false;
    convert_from_opts_allow_resize_csi(py_opts, opts);
    if (PyErr_Occurred()) return false;
