...
new_screen_object(PyTypeObject *type, PyObject *args, PyObject UNUSED *kwds)
{
    ...
    id_type window_id = 0;
    if (!PyArg_ParseTuple(args, "|OIIIIIKO", &callbacks, &lines, &columns, &scrollback, &cell_width, &cell_height, &window_id, &test_child))
        return NULL;
...
do_screen_reset(Screen *self, bool is_hard_reset)
{
    ...
    if (is_hard_reset && self->linebuf == self->alt_linebuf)
...
void screen_reset(Screen *self) { do_screen_reset(self, true); }
...
rewrap(Screen *screen, unsigned int lines, unsigned int columns, index_type *nclb, index_type *ncla, CursorTrack *cursor, CursorTrack *main_saved_cursor, CursorTrack *alt_saved_cursor, bool main_is_active)
{
    ...
    if (main_is_active)
        cursors[1] = (TrackCursor){.x = cursor->before.x, .y = cursor->before.y};
    else
        cursors[1].is_sentinel = true;
    ...
    for (size_t i = 0; i < selections->count; i++)
    {
        ...
        if (up)
        {
            ...
            else
            {
                ...
                if (s->input_start.y)
                    s->input_start.y--;
                if (s->input_current.y)
                    s->input_current.y--;
                if (s->initial_extent.start.y)
                    s->initial_extent.start.y--;
                if (s->initial_extent.end.y)
                    s->initial_extent.end.y--;
            }
            if (s->end.y == 0)
                s->end_scrolled_by += 1;
            else
        ...
        {
            if (s->start.y >= self->lines - 1)
                s->start_scrolled_by -= 1;
            else
            {
                s->start.y++;
                if (s->input_start.y < self->lines - 1)
                    s->input_start.y++;
                if (s->input_current.y < self->lines - 1)
                    s->input_current.y++;
            }
            if (s->end.y >= self->lines - 1)
                s->end_scrolled_by -= 1;
            else
                s->end.y++;
        }
...
#define INDEX_DOWN                                                               \
    ...
    {                                                                            \
        if (self->last_visited_prompt.scrolled_by > 0)                           \
            self->last_visited_prompt.scrolled_by--;                             \
        else if (self->last_visited_prompt.y < self->lines - 1)                  \
            self->last_visited_prompt.y++;                                       \
        else                                                                     \
...
nuke_multicell_char_at(Screen *self, index_type x_, index_type y_, bool replace_with_spaces)
{
    ...
    if (self->linebuf == self->main_linebuf)
        y_min_limit = -(self->historybuf->count + 1);
    for (int y = (int)y_ - 1; y > y_min_limit && num_lines_above; y--, num_lines_above--)
    {
        ...
        if (y > -1)
            ...
            historybuf_mark_line_dirty(self->historybuf, -(y + 1));
    }
    self->is_dirty = true;
}
...
nuke_multiline_char_intersecting_with(Screen *self, index_type x_start, index_type x_limit, index_type y_start, index_type y_limit, bool replace_with_spaces)
{
    for (index_type y = y_start; y < y_limit; y++)
    {
        ...
        {
            if (cp[x].is_multicell && cp[x].scale > 1)
    ...
    Screen *self, index_type start, index_type limit, index_type y, bool replace_with_spaces)
{
    ...
    for (index_type x = start; x < limit; x++)
    {
        if (cpu_cells[x].is_multicell)
        {
            ...
            if (cpu_cells[x].x || mcd_x_limit > limit)
                nuke_in_line(cpu_cells, gpu_cells, x, MIN(mcd_x_limit, limit), replace_with_spaces ? ' ' : 0);
            x = mcd_x_limit - 1;
        }
    }
}
...
prevent_current_prompt_from_rewrapping(Screen *self, LineBuf *prompt_copy, index_type *num_of_prompt_lines_above_cursor)
{
    ...
    *num_of_prompt_lines_above_cursor = 0;
    if (!self->prompt_settings.redraws_prompts_at_all)
...
screen_resize(Screen *self, unsigned int lines, unsigned int columns)
{
    ...
    if (is_main)
    {
        ...
        if (self->alt_savepoint.is_valid)
            alt_has_blank_line = preserve_blank_output_start_line(&self->alt_savepoint.cursor, self->alt_linebuf);
    }
    else
    {
        if (self->main_savepoint.is_valid)
            main_has_blank_line = preserve_blank_output_start_line(&self->main_savepoint.cursor, self->main_linebuf);
...
#define setup_cursor(which)                                                                                    \
    ...
    if (!init_overlay_line(self, columns, true))
        return false;
...
#undef S
    ...
    if (main_has_blank_line)
        remove_blank_output_line_reservation_marker(is_main ? self->cursor : &self->main_savepoint.cursor, self->main_linebuf);
    if (alt_has_blank_line)
        remove_blank_output_line_reservation_marker(is_main ? &self->alt_savepoint.cursor : self->cursor, self->alt_linebuf);
...
clear_intersecting_selections(Screen *self, index_type y)
{
    if (selection_has_screen_line(&self->selections, y))
        clear_selection(&self->selections);
    if (selection_has_screen_line(&self->url_ranges, y))
        clear_selection(&self->url_ranges);
}
...
init_prev_cell(Screen *self, text_loop_state *s)
{
    ...
    else if (self->cursor->y)
    {
        ...
        s->prev.cc = linebuf_cpu_cell_at(self->linebuf, s->prev.x, s->prev.y);
        if (!s->prev.cc->next_char_was_wrapped)
            s->prev.cc = NULL;
    }
}
static void
init_segmentation_state(Screen *self, text_loop_state *s)
{
    ...
    if (s->prev.cc)
    {
        ...
        for (index_type i = 0; i < self->lc->count; i++)
            s->seg = grapheme_segmentation_step(s->seg, char_props_for(self->lc->chars[i]));
    }
}
...
visual_line_is_continued(Screen *self, int y_)
{
    ...
    {
        if (y < self->scrolled_by)
            return historybuf_is_line_continued(self->historybuf, self->scrolled_by - 1 - y);
        y -= self->scrolled_by;
    }
    if (y)
        return linebuf_is_line_continued(self->linebuf, y);
...
{
    if (y < 0)
        historybuf_init_line(self->historybuf, -(y + 1), line);
    else
        init_line_(self, y, line);
}
...
{
    if (-(int)self->historybuf->count <= y && y < (int)self->lines)
...
range_line_is_continued(Screen *self, int y)
{
    if (!(-(int)self->historybuf->count <= y && y < (int)self->lines))
        return false;
    if (y < 0)
        return historybuf_is_line_continued(self->historybuf, -(y + 1));
    if (y)
        return linebuf_is_line_continued(self->linebuf, y);
    return self->linebuf == self->main_linebuf ? history_buf_endswith_wrap(self->historybuf) : false;
}
...
halve_multicell_width(Screen *self, index_type x_, index_type y_)
{
    ...
    if (expected_y_min_limit < y_min_limit)
        return false;
    ...
    for (int y = y_min_limit + 1; y < y_max_limit; y++)
    {
        ...
        cp = line->cpu_cells;
        gp = line->gpu_cells;
        for (index_type x = x_; x < half_x_limit; x++)
            cp[x].width = new_width;
...
void set_active_hyperlink(Screen *self, char *id, char *url)
{
...
text_cache_gc_process_cells(TextCache *tc, TextCacheGCData *gc, CPUCell *cells, size_t count)
{
    for (size_t i = 0; i < count; i++)
    {
        ...
        if (!c->ch_is_idx)
            continue;
        char_type new_idx;
        if (tc_gc_map_index(tc, gc, c->ch_or_idx, &new_idx))
            c->ch_or_idx = new_idx;
        else
            cell_set_char(c, 0); // stale index, should not happen
    }
}
...
{
    if (lb)
        text_cache_gc_process_cells(tc, gc, lb->cpu_cell_buf, (size_t)lb->ynum * lb->xnum);
}

void screen_garbage_collect_text_cache(Screen *self)
{
    ...
    if (!gc)
        return; // allocation failure, cache left unchanged
    ...
    if (self->overlay_line.cpu_cells)
        text_cache_gc_process_cells(
...
add_combining_char(Screen *self, char_type ch, index_type x, index_type y)
{
    ...
    if (!cell_has_text(cell) || (cell->is_multicell && cell->y))
        return false; // don't allow adding combining chars to a null cell
    text_in_cell(cell, self->text_cache, self->lc);
    if (self->lc->count >= MAX_NUM_CODEPOINTS_PER_CELL)
        return false; // don't allow too many combining chars to prevent DoS attacks
    ...
    if (cell->is_multicell)
    {
        ...
        for (index_type v = y; v < y + cell->scale; v++)
        {
            ...
            for (index_type h = x; h < x_limit; h++)
                cpu_cells[h].ch_and_idx = ch_and_idx;
...
{
    if (required_width > self->columns)
        return false;
    ...
    while (true)
    {
        ...
        while (self->cursor->x + required_width <= self->columns)
        {
            ...
            {
                if (cp[self->cursor->x].is_multicell)
                    nuke_multicell_char_at(self, self->cursor->x, self->cursor->y, cp[self->cursor->x].x != 0);
                return true;
            }
            self->cursor->x++;
        }
...
draw_combining_char(Screen *self, text_loop_state *s, char_type ch)
{
    ...
    while (xpos && cp[xpos].is_multicell && cp[xpos].x)
        xpos--;
    if (!add_combining_char(self, ch, xpos, s->prev.y) || self->lc->count < 2)
        return;
    unsigned base_pos = self->lc->count - 2;
    if (ch == VS16)
    { // emoji presentation variation marker makes default text presentation emoji (narrow emoji) into wide emoji
...
void screen_designate_charset(Screen *self, uint32_t which, uint32_t as)
{
    ...
    case 0:
        ...
        if (self->charset.current_num == 0)
            self->charset.current = self->charset.zero;
        break;
    case 1:
        self->charset.one = translation_table(as);
        if (self->charset.current_num == 1)
            self->charset.current = self->charset.one;
        break;
    }
}
...
draw_control_char(Screen *self, text_loop_state *s, uint32_t ch)
{
    ...
    case HT:
            ...
            else if (self->columns > 0)
            {
                ...
                if (s->cp[self->cursor->x].is_multicell)
                {
                    if (s->cp[self->cursor->x].y)
                        ...
                        replace_multicell_char_under_cursor_with_spaces(self);
                }
                screen_tab(self);
            }
        }
        ...
        break;
    case SI:
        screen_change_charset(self, 0);
        break;
    case SO:
        screen_change_charset(self, 1);
        break;
    case LF:
    case VT:
    case FF:
...
draw_text_loop(Screen *self, const uint32_t *chars, size_t num_chars, text_loop_state *s)
{
    ...
    for (size_t i = 0; i < num_chars; i++)
    {
        ...
        if (ch < DEL && s->seg.grapheme_break <= GBP_None)
        ...
        if (self->cursor->x < self->columns && s->cp[self->cursor->x].is_multicell)
        {
            if (s->cp[self->cursor->x].y)
            {
                ...
                nuke_multicell_char_at(self, self->cursor->x, self->cursor->y, s->cp[self->cursor->x].x != 0);
        }

        self->last_graphic_char = ch;
        if (UNLIKELY(self->columns < self->cursor->x + (unsigned int)char_width))
        {
            if ((unsigned)char_width > self->columns)
                continue; // discard too wide character
            if (self->modes.mDECAWM)
            {
                continue_to_next_line(self);
                init_text_loop_line(self, s);
        ...
        if (self->modes.mIRM)
            insert_characters(self, self->cursor->x, char_width, self->cursor->y, true);
        ...
        if (char_width == 2)
        {
            ...
            if (second->is_multicell)
            {
                if (second->y)
                {
                    ...
                    nuke_multicell_char_at(self, self->cursor->x + 1, self->cursor->y, true);
            }
...
#define PREPARE_FOR_DRAW_TEXT                                                                                                       \
    if (tc_should_gc(self->text_cache))                                                                                             \
        screen_garbage_collect_text_cache(self);                                                                                    \
    ...
    if (force_underline)                                                                                                            \
        attrs.decoration = OPT(url_style);                                                                                          \
    text_loop_state s = {                                                                                                           \
...
handle_fixed_width_multicell_command(Screen *self, CPUCell mcd, ListOfChars *lc)
{
    ...
    if (width > self->columns || height > max_height)
        return;
    ...
    if (self->modes.mIRM)
    {
        ...
        {
            if (self->modes.mIRM)
                insert_characters(self, self->cursor->x, width, y, true);
        }
    }
    for (index_type y = self->cursor->y; y < self->cursor->y + height; y++)
    {
        ...
        {
            if (s.cp[x].is_multicell)
                nuke_multicell_char_at(self, x, y, s.cp[x].x + s.cp[x].y > 0);
            s.cp[x] = mcd;
            s.gp[x] = s.g;
        }
    }
...
#define M(x) ((1u << x) - 1u)
    CPUCell mcd = {
        .width = MIN(cmd->width, M(WIDTH_BITS)), .scale = MAX(1u, MIN(cmd->scale, M(SCALE_BITS))), .subscale_n = MIN(cmd->subscale_n, M(SUBSCALE_BITS)), .subscale_d = MIN(cmd->subscale_d, M(SUBSCALE_BITS)), .valign = MIN(cmd->vertical_align, M(VALIGN_BITS)), .halign = MIN(cmd->horizontal_align, M(HALIGN_BITS)), .is_multicell = true};
#undef M
    if (mcd.width)
        handle_fixed_width_multicell_command(self, mcd, self->lc);
    else
    {
        RAII_ListOfChars(lc);
        ...
        for (unsigned i = 0; i < self->lc->count; i++)
        {
            ...
            {
                if (lc.count)
                    handle_variable_width_multicell_command(self, mcd, &lc);
...
void select_graphic_rendition(Screen *self, int *params, unsigned int count, bool is_group, Region *region_)
{
    if (region_)
    {
...
write_to_child(Screen *self, const char *data, size_t sz)
{
    bool written = false;
    if (self->window_id)
        written = schedule_write_to_child(self->window_id, 1, data, sz);
...
void screen_visibility_changed(Screen *self, bool potentially_visible)
{
    ...
    if (self->visibility_state != state)
    {
        ...
        if (self->modes.mVISIBILITY_REPORTS)
            send_visibility_report(self);
    }
}
...
write_escape_code_to_child_python(Screen *self, unsigned char which, PyObject *data)
{
    ...
    get_prefix_and_suffix_for_escape_code(which, &prefix, &suffix);
    if (self->window_id)
        written = schedule_write_to_child_python(self->window_id, prefix, data, suffix);
    if (self->test_child != Py_None)
    {
        write_to_test_child(self, prefix, strlen(prefix));
        for (Py_ssize_t i = 0; i < PyTuple_GET_SIZE(data); i++)
        {
            ...
            if (PyBytes_Check(t))
                write_to_test_child(self, PyBytes_AS_STRING(t), PyBytes_GET_SIZE(t));
            else
            {
                Py_ssize_t sz;
                const char *d = PyUnicode_AsUTF8AndSize(t, &sz);
                if (d)
                    write_to_test_child(self, d, sz);
            }
        }
        if (suffix[0])
            write_to_test_child(self, suffix, strlen(suffix));
...
{
    Window *w;
    if ((!self->current_hyperlink_under_mouse.id && !self->current_hyperlink_under_mouse.has_detected_url) || !self->window_id || !(w = window_for_window_id(self->window_id)))
        return false;
...
void screen_handle_dnd_command(Screen *self, const DnDCommand *cmd_, const uint8_t *payload)
{
    Window *w;
    if (!self->window_id || !(w = window_for_window_id(self->window_id)))
        return;
    ...
    case 'a':
        if (cmd->cell_x == 1)
            drop_register_machine_id(w, payload, cmd->payload_sz);
        else
            drop_register_window(w, payload, cmd->payload_sz, true, cmd->client_id, cmd->more);
        break;
    case 'A':
        drop_register_window(w, NULL, 0, false, cmd->client_id, cmd->more);
        break;
    case 'm':
        drop_set_status(w, cmd->operation, (const char *)payload, cmd->payload_sz, cmd->more);
        break;
    case 'r':
    {
    ...
    case 'o':
    {
        ...
        {
        case 1:
            drag_start_offerring(w, (const char *)payload, cmd->payload_sz);
            break;
        case 2:
            drag_stop_offerring(w);
            break;
        case 0:
            drag_add_mimes(
    ...
    {
        if (cmd->cell_x >= 0)
            drag_add_pre_sent_data(w, cmd->cell_x, payload, cmd->payload_sz);
        else
            drag_add_image(w, -cmd->cell_x, cmd->cell_y, cmd->pixel_x, cmd->pixel_y, (int)cmd->operation, payload, cmd->payload_sz);
    }
    break;
    case 'P':
    {
        if (cmd->cell_x >= 0)
            drag_change_image(w, cmd->cell_x);
        else
            drag_start(w);
    }
    break;
...
void screen_toggle_screen_buffer(Screen *self, bool save_cursor, bool clear_alt_screen)
{
...
#define SIMPLE_MODE(name)          \
    case name:                     \
        self->modes.m##name = val; \
        break;

#define MOUSE_MODE(name, attr, value)       \
    case name:                              \
        self->modes.attr = val ? value : 0; \
        break;
    ...
    case INBAND_RESIZE_NOTIFICATION:
        ...
        if (val)
            CALLBACK("notify_child_of_resize", NULL);
        break;
...
void screen_decsace(Screen *self, unsigned int val)
{
...
void screen_modify_other_keys(Screen *self, unsigned val, unsigned val2)
{
...
screen_current_key_encoding_flags(Screen *self)
{
    for (unsigned i = arraysz(self->main_key_encoding_flags); i-- > 0;)
    {
        if (self->key_encoding_flags[i] & 0x80)
            return self->key_encoding_flags[i] & 0x7f;
    }
    return 0;
}
...
void screen_push_key_encoding_flags(Screen *self, uint32_t val)
{
    ...
    if (current_idx == sz - 1)
        memmove(self->key_encoding_flags, self->key_encoding_flags + 1, (sz - 1) * sizeof(self->main_key_encoding_flags[0]));
    else
        self->key_encoding_flags[current_idx++] |= 0x80;
...
screen_pointer_shape(Screen *self)
{
    if (self->linebuf == self->main_linebuf)
    {
        if (self->main_pointer_shape_stack.count)
            return self->main_pointer_shape_stack.stack[self->main_pointer_shape_stack.count - 1];
    }
    else
    {
        if (self->alternate_pointer_shape_stack.count)
            return self->alternate_pointer_shape_stack.stack[self->alternate_pointer_shape_stack.count - 1];
    }
...
change_pointer_shape(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "ss", &b, &css_name))
        return NULL;
    op = b[0];
    uint8_t *count, *stack;
    ...
    else
    {
        ...
        else if (strcmp("default", css_name) == 0)
            s = DEFAULT_POINTER;
        else if (strcmp("text", css_name) == 0)
            s = TEXT_POINTER;
        else if (strcmp("pointer", css_name) == 0)
            s = POINTER_POINTER;
        else if (strcmp("help", css_name) == 0)
            s = HELP_POINTER;
        else if (strcmp("wait", css_name) == 0)
            s = WAIT_POINTER;
        else if (strcmp("progress", css_name) == 0)
            s = PROGRESS_POINTER;
        else if (strcmp("crosshair", css_name) == 0)
            s = CROSSHAIR_POINTER;
        else if (strcmp("cell", css_name) == 0)
            s = CELL_POINTER;
        else if (strcmp("vertical-text", css_name) == 0)
            s = VERTICAL_TEXT_POINTER;
        else if (strcmp("move", css_name) == 0)
            s = MOVE_POINTER;
        else if (strcmp("e-resize", css_name) == 0)
            s = E_RESIZE_POINTER;
        else if (strcmp("ne-resize", css_name) == 0)
            s = NE_RESIZE_POINTER;
        else if (strcmp("nw-resize", css_name) == 0)
            s = NW_RESIZE_POINTER;
        else if (strcmp("n-resize", css_name) == 0)
            s = N_RESIZE_POINTER;
        else if (strcmp("se-resize", css_name) == 0)
            s = SE_RESIZE_POINTER;
        else if (strcmp("sw-resize", css_name) == 0)
            s = SW_RESIZE_POINTER;
        else if (strcmp("s-resize", css_name) == 0)
            s = S_RESIZE_POINTER;
        else if (strcmp("w-resize", css_name) == 0)
            s = W_RESIZE_POINTER;
        else if (strcmp("ew-resize", css_name) == 0)
            s = EW_RESIZE_POINTER;
        else if (strcmp("ns-resize", css_name) == 0)
            s = NS_RESIZE_POINTER;
        else if (strcmp("nesw-resize", css_name) == 0)
            s = NESW_RESIZE_POINTER;
        else if (strcmp("nwse-resize", css_name) == 0)
            s = NWSE_RESIZE_POINTER;
        else if (strcmp("zoom-in", css_name) == 0)
            s = ZOOM_IN_POINTER;
        else if (strcmp("zoom-out", css_name) == 0)
            s = ZOOM_OUT_POINTER;
        else if (strcmp("alias", css_name) == 0)
            s = ALIAS_POINTER;
        else if (strcmp("copy", css_name) == 0)
            s = COPY_POINTER;
        else if (strcmp("not-allowed", css_name) == 0)
            s = NOT_ALLOWED_POINTER;
        else if (strcmp("no-drop", css_name) == 0)
            s = NO_DROP_POINTER;
        else if (strcmp("grab", css_name) == 0)
            s = GRAB_POINTER;
        else if (strcmp("grabbing", css_name) == 0)
            s = GRABBING_POINTER;
        else if (strcmp("left_ptr", css_name) == 0)
            s = DEFAULT_POINTER;
        else if (strcmp("xterm", css_name) == 0)
            s = TEXT_POINTER;
        else if (strcmp("ibeam", css_name) == 0)
            s = TEXT_POINTER;
        else if (strcmp("pointing_hand", css_name) == 0)
            s = POINTER_POINTER;
        else if (strcmp("hand2", css_name) == 0)
            s = POINTER_POINTER;
        else if (strcmp("hand", css_name) == 0)
            s = POINTER_POINTER;
        else if (strcmp("question_arrow", css_name) == 0)
            s = HELP_POINTER;
        else if (strcmp("whats_this", css_name) == 0)
            s = HELP_POINTER;
        else if (strcmp("clock", css_name) == 0)
            s = WAIT_POINTER;
        else if (strcmp("watch", css_name) == 0)
            s = WAIT_POINTER;
        else if (strcmp("half-busy", css_name) == 0)
            s = PROGRESS_POINTER;
        else if (strcmp("left_ptr_watch", css_name) == 0)
            s = PROGRESS_POINTER;
        else if (strcmp("tcross", css_name) == 0)
            s = CROSSHAIR_POINTER;
        else if (strcmp("plus", css_name) == 0)
            s = CELL_POINTER;
        else if (strcmp("cross", css_name) == 0)
            s = CELL_POINTER;
        else if (strcmp("fleur", css_name) == 0)
            s = MOVE_POINTER;
        else if (strcmp("pointer-move", css_name) == 0)
            s = MOVE_POINTER;
        else if (strcmp("right_side", css_name) == 0)
            s = E_RESIZE_POINTER;
        else if (strcmp("top_right_corner", css_name) == 0)
            s = NE_RESIZE_POINTER;
        else if (strcmp("top_left_corner", css_name) == 0)
            s = NW_RESIZE_POINTER;
        else if (strcmp("top_side", css_name) == 0)
            s = N_RESIZE_POINTER;
        else if (strcmp("bottom_right_corner", css_name) == 0)
            s = SE_RESIZE_POINTER;
        else if (strcmp("bottom_left_corner", css_name) == 0)
            s = SW_RESIZE_POINTER;
        else if (strcmp("bottom_side", css_name) == 0)
            s = S_RESIZE_POINTER;
        else if (strcmp("left_side", css_name) == 0)
            s = W_RESIZE_POINTER;
        else if (strcmp("sb_h_double_arrow", css_name) == 0)
            s = EW_RESIZE_POINTER;
        else if (strcmp("split_h", css_name) == 0)
            s = EW_RESIZE_POINTER;
        else if (strcmp("sb_v_double_arrow", css_name) == 0)
            s = NS_RESIZE_POINTER;
        else if (strcmp("split_v", css_name) == 0)
            s = NS_RESIZE_POINTER;
        else if (strcmp("size_bdiag", css_name) == 0)
            s = NESW_RESIZE_POINTER;
        else if (strcmp("size-bdiag", css_name) == 0)
            s = NESW_RESIZE_POINTER;
        else if (strcmp("size_fdiag", css_name) == 0)
            s = NWSE_RESIZE_POINTER;
        else if (strcmp("size-fdiag", css_name) == 0)
            s = NWSE_RESIZE_POINTER;
        else if (strcmp("zoom_in", css_name) == 0)
            s = ZOOM_IN_POINTER;
        else if (strcmp("zoom_out", css_name) == 0)
            s = ZOOM_OUT_POINTER;
        else if (strcmp("dnd-link", css_name) == 0)
            s = ALIAS_POINTER;
        else if (strcmp("dnd-copy", css_name) == 0)
            s = COPY_POINTER;
        else if (strcmp("forbidden", css_name) == 0)
            s = NOT_ALLOWED_POINTER;
        else if (strcmp("crossed_circle", css_name) == 0)
            s = NOT_ALLOWED_POINTER;
        else if (strcmp("dnd-no-drop", css_name) == 0)
            s = NO_DROP_POINTER;
        else if (strcmp("openhand", css_name) == 0)
            s = GRAB_POINTER;
        else if (strcmp("hand1", css_name) == 0)
            s = GRAB_POINTER;
        else if (strcmp("closedhand", css_name) == 0)
            s = GRABBING_POINTER;
        else if (strcmp("dnd-none", css_name) == 0)
            s = GRABBING_POINTER;
        /* end css to enum */
...
void screen_backtab(Screen *self, unsigned int count)
{
    ...
    else
    {
        ...
        while (count > 0)
        {
            ...
            else
            {
                ...
                {
                    if (self->cursor->y == top || !allow_move_to_previous_line)
                        count = 0;
                    else
                    {
                        count--;
...
#define SIMPLE_MODE(name)             \
    case name:                        \
        dest->m##name = src->m##name; \
        break;
#define SIDE_EFFECTS(name)                                 \
    case name:                                             \
        if (do_side_effects)                               \
            set_mode_from_const(self, name, src->m##name); \
        else                                               \
            dest->m##name = src->m##name;                  \
        break;
    ...
    case MOUSE_MOVE_TRACKING:
        dest->mouse_tracking_mode = src->mouse_tracking_mode;
        break;
    case MOUSE_UTF8_MODE:
    case MOUSE_SGR_MODE:
    case MOUSE_URXVT_MODE:
        dest->mouse_tracking_protocol = src->mouse_tracking_protocol;
        break;
    case DECSCLM:
    case DECNRCM:
        break; // we ignore these modes
...
void screen_save_mode(Screen *self, unsigned int mode)
{ // XTSAVE
    copy_specific_mode(self, mode, &self->modes, &self->saved_modes);
}

void screen_restore_mode(Screen *self, unsigned int mode)
{ // XTRESTORE
...
void screen_save_modes(Screen *self)
{
...
void screen_restore_modes(Screen *self)
{
...
void screen_ensure_bounds(Screen *self, bool force_use_margins /*=false*/, bool in_margins)
{
...
int screen_cursor_at_a_shell_prompt(const Screen *self)
{
    if (self->cursor->y >= self->lines || self->linebuf != self->main_linebuf || !screen_is_cursor_visible(self))
        return -1;
...
bool screen_prompt_supports_click_events(const Screen *self, bool *is_relative)
{
    *is_relative = (bool)self->prompt_settings.relative_click_events;
    return (bool)self->prompt_settings.supports_click_events;
}

bool screen_fake_move_cursor_to_position(Screen *self, index_type start_x, index_type start_y)
{
    ...
    if (count)
    {
        ...
        if (self->prompt_settings.uses_special_keys_for_cursor_movement)
        {
            ...
            for (unsigned i = 0; i < count; i++)
                write_to_child(self, output, num);
        }
        else
        {
...
screen_clear_scrollback(Screen *self)
{
    ...
    {
        CPUCell *c = cells + x;
        if (c->is_multicell && c->y > 0)
        { // multiline char that extended into scrollback
...
{
    if (self->linebuf != self->main_linebuf || self->margin_top != 0 || self->margin_bottom != self->lines - 1)
        return;
...
void screen_erase_in_display(Screen *self, unsigned int how, bool private)
{
    ...
    */
    ...
    {
        if (how != 3)
            screen_dirty_line_graphics(self, a, b, self->linebuf == self->main_linebuf);
        if (private)
        {
            ...
            linebuf_clear_lines(self->linebuf, self->cursor, a, b);
        if (nuke_multicell_chars)
            nuke_multicell_char_intersecting_with(self, 0, self->columns, a, b, false);
        self->is_dirty = true;
        if (selection_intersects_screen_lines(&self->selections, a, b))
            clear_selection(&self->selections);
        if (selection_intersects_screen_lines(&self->url_ranges, a, b))
            clear_selection(&self->url_ranges);
    }
    if (how < 2)
    {
        ...
        if (how == 1)
            linebuf_clear_attrs_and_dirty(self->linebuf, self->cursor->y);
    }
...
void screen_insert_lines(Screen *self, unsigned int count)
{
    ...
    if (top <= self->cursor->y && self->cursor->y <= bottom)
    {
        ...
        for (index_type x = 0; x < self->columns; x++)
        {
            if (cells[x].is_multicell && cells[x].y)
                nuke_multicell_char_at(self, x, self->cursor->y, false);
        }
...
screen_scroll_until_cursor_prompt(Screen *self, bool add_to_scrollback)
{
    ...
    if (add_to_scrollback)
        ...
        while (num_lines_to_scroll--)
            screen_index_without_adding_to_history(self);
...
void screen_repeat_character(Screen *self, unsigned int count)
{
    if (self->last_graphic_char)
    {
        ...
        for (unsigned i = 0; i < arraysz(buf); i++)
            buf[i] = self->last_graphic_char;
        for (unsigned i = 0; i < num; i += arraysz(buf))
            screen_draw_text(self, buf, MIN(num - i, arraysz(buf)));
    }
}
...
void report_device_attributes(Screen *self, unsigned int mode, char start_modifier)
{
    if (mode == 0)
    {
...
void screen_xtversion(Screen *self, unsigned int mode)
{
    if (mode == 0)
    {
...
void screen_report_size(Screen *self, unsigned which, unsigned modifier)
{
    char buf[32] = {0};
...
void screen_manipulate_title_stack(Screen *self, unsigned int op, unsigned int which)
{
...
void report_device_status(Screen *self, unsigned int which, bool private)
{
    unsigned int x, y;
    static char buf[64];
    switch (which)
...
void report_mode_status(Screen *self, unsigned int which, bool private)
{
...
#undef KNOWN_MODE
    case ALTERNATE_SCREEN:
        ans = self->linebuf == self->alt_linebuf ? 1 : 2;
        break;
    case MOUSE_BUTTON_TRACKING:
        ans = self->modes.mouse_tracking_mode == BUTTON_MODE ? 1 : 2;
        break;
    case MOUSE_MOTION_TRACKING:
        ans = self->modes.mouse_tracking_mode == MOTION_MODE ? 1 : 2;
        break;
    case MOUSE_MOVE_TRACKING:
        ans = self->modes.mouse_tracking_mode == ANY_MODE ? 1 : 2;
        break;
    case MOUSE_SGR_MODE:
        ans = self->modes.mouse_tracking_protocol == SGR_PROTOCOL ? 1 : 2;
        break;
    case MOUSE_UTF8_MODE:
        ans = self->modes.mouse_tracking_protocol == UTF8_PROTOCOL ? 1 : 2;
        break;
    case MOUSE_SGR_PIXEL_MODE:
        ans = self->modes.mouse_tracking_protocol == SGR_PIXEL_PROTOCOL ? 1 : 2;
        break;
    case PENDING_UPDATE:
        ans = self->paused_rendering.expires_at ? 1 : 2;
        break;
    }
...
void screen_set_cursor(Screen *self, unsigned int mode, uint8_t secondary)
{
    ...
    case ' ': // DECSCUSR
        ...
        {
            blink = mode % 2;
            shape = (mode < 3) ? CURSOR_BLOCK : (mode < 5) ? CURSOR_UNDERLINE
                                            : (mode < 7)   ? CURSOR_BEAM
                                                           : NO_CURSOR_SHAPE;
        }
...
#define o()                                                                              \
    ...
    case COLOR_IS_INDEX:                                                                 \
        ...
        break;                                                                           \
    case COLOR_IS_RGB:                                                                   \
        pr("2:%u:%u:%u", (ecc.rgb >> 16) & 0xff, (ecc.rgb >> 8) & 0xff, ecc.rgb & 0xff); \
        break;                                                                           \
    }
...
#undef pr
    ...
    if (queried_shape == 30 || queried_shape == 40)
    {
        ...
        {
        case 0:
            ecc->type = COLOR_NOT_SET;
            break;
        case 1:
            ecc->type = COLOR_IS_SPECIAL;
            break;
        case 2:
            if (num_params > 3)
            {
    ...
    case 2:
    {
        ...
        for (unsigned i = 0; i + 1 < num_params; i += 2)
        {
            ...
            else if (y < self->lines && x < self->columns)
                vt_insert(&s, y * self->columns + x, shape);
        }
    ...
    case 4:
    {
        ...
        for (unsigned i = 0; i < self->extra_cursors.count; i++)
        {
            ...
            if (!in_some_region)
                self->extra_cursors.locations[count++] = self->extra_cursors.locations[i];
        }
...
void desktop_notify(Screen *self, unsigned int osc_code, PyObject *data)
{
...
void color_control(Screen *self, unsigned int code, PyObject *spec)
{
    if (spec)
        CALLBACK("color_control", "IO", code, spec);
}
...
void file_transmission(Screen *self, PyObject *data)
{
...
parse_prompt_mark(Screen *self, char *buf, PromptKind *pk)
{
    ...
    while (true)
    {
        ...
        if (token == NULL)
            return;
        if (strcmp(token, "k=s") == 0)
            *pk = SECONDARY_PROMPT;
        else if (strcmp(token, "redraw=0") == 0)
            self->prompt_settings.redraws_prompts_at_all = 0;
        else if (strcmp(token, "special_key=1") == 0)
            self->prompt_settings.uses_special_keys_for_cursor_movement = 1;
...
#define ensure_y_ok                                                 \
    if (y >= (int)self->lines || -y > (int)self->historybuf->count) \
        return false;
        ensure_y_ok;
        y += scroll_offset;
...
void process_cwd_notification(Screen *self, unsigned int code, const char *data, size_t sz)
{
    if (code == 7)
    {
...
bool screen_send_signal_for_key(Screen *self, char key)
{
    int ret = 0;
    if (self->callbacks != Py_None)
    {
...
void screen_push_colors(Screen *self, unsigned int idx)
{
    if (colorprofile_push_colors(self->color_profile, idx))
        self->color_profile->dirty = true;
}

void screen_pop_colors(Screen *self, unsigned int idx)
{
...
void screen_report_color_stack(Screen *self)
{
...
void screen_request_capabilities(Screen *self, char c, const char *query)
{
    static char buf[128];
    int shape = 0;
    switch (c)
    ...
    case '$':
        ...
        if (strcmp(" q", query) == 0)
        {
            ...
            case NUM_OF_CURSOR_SHAPES:
                ...
                break;
            case CURSOR_BLOCK:
                shape = self->cursor->non_blinking ? 2 : 0;
                break;
            case CURSOR_UNDERLINE:
                shape = self->cursor->non_blinking ? 4 : 3;
                break;
            case CURSOR_BEAM:
                shape = self->cursor->non_blinking ? 6 : 5;
                break;
            }
        ...
        if (shape > 0)
            write_escape_code_to_child(self, ESC_DCS, buf);
        break;
    }
}
...
copy_selections(Selections *dest, const Selections *src)
{
    ...
    for (unsigned i = 0; i < dest->count; i++)
        memcpy(dest->items + i, src->items + i, sizeof(dest->items[0]));
...
bool screen_pause_rendering(Screen *self, bool pause, int for_in_ms)
{
    ...
    if (self->paused_rendering.expires_at)
        return false;
    if (!self->paused_rendering.grman)
        self->paused_rendering.grman = grman_alloc(true);
    if (!self->paused_rendering.grman)
        return false;
    if (for_in_ms <= 0)
        for_in_ms = 2000;
...
bool get_line_edge_colors_at_row(Screen *self, index_type y, color_type *left, color_type *right, bool *left_is_default, bool *right_is_default)
{
    ...
    if (left_is_default)
        *left_is_default = (line->gpu_cells[cell_color_x].bg & 0xff) == 0;
    if (left)
        *left = effective_cell_edge_color(left_char, left_cell_fg, left_cell_bg, true);
    if (line->xnum > 0)
        cell_color_x = line->xnum - 1;
    ...
    if (right_is_default)
        *right_is_default = (line->gpu_cells[cell_color_x].bg & 0xff) == 0;
    if (right)
    ...
    for (i = 0; i < line->xnum; i++)
    {
        ...
        else
        {
            ...
            if (cell_first_char(cpu_cell, self->text_cache) == IMAGE_PLACEHOLDER_CHAR)
            {
                ...
                if (!cur_img_row)
                    cur_img_row = 1;
                if (!cur_img_id_higher8bits)
                    cur_img_id_higher8bits = 1;
            }
        }
...
void screen_update_cell_data(Screen *self, void *address, FONTS_DATA_HANDLE fonts_data, bool cursor_has_moved)
{
    if (self->paused_rendering.expires_at)
    {
        if (!self->paused_rendering.cell_data_updated)
        {
            ...
            for (index_type y = 0; y < self->lines; y++)
            {
                ...
                if (linebuf->line->attrs.has_dirty_text)
                {
                    ...
                    if (linebuf->line->attrs.has_dirty_text && screen_has_marker(self))
                        mark_text_in_line(
    ...
    for (unsigned int render_row = 0; render_row < render_lines; render_row++)
    {
        ...
        if (is_history)
        {
            ...
            if (force_history_render || linep->attrs.has_dirty_text)
            {
                ...
                if (screen_has_marker(self))
                    mark_text_in_line(self->marker, linep, &self->as_ansi_buf);
                historybuf_mark_line_clean(self->historybuf, lnum);
            }
        }
        else
        {
                ...
                (cursor_has_moved && (self->cursor->y == lnum || self->last_rendered.cursor.y == lnum)))
            {
                render_line(fonts_data, linep, lnum, self->cursor, self->disable_ligatures, self->lc);
                screen_render_line_graphics(self, linep, virtual_y - (int)self->scrolled_by);
                if (linep->attrs.has_dirty_text && screen_has_marker(self))
                    mark_text_in_line(
                        self->marker, linep, &self->as_ansi_buf);
                if (is_overlay_active && lnum == self->overlay_line.ynum)
                    render_overlay_line(self, linep, fonts_data);
                linebuf_mark_line_clean(self->linebuf, lnum);
            }
        }
...
selection_boundary_less_than(const SelectionBoundary *a, const SelectionBoundary *b)
{
    ...
    if (a->x > b->x)
        return false;
    if (a->in_left_half_of_cell && !b->in_left_half_of_cell)
        return true;
    return false;
}
...
num_cells_between_selection_boundaries(const Screen *self, const SelectionBoundary *a, const SelectionBoundary *b)
{
    ...
    index_type ans = 0;
    if (before->y + 1 < after->y)
        ans += self->columns * (after->y - before->y - 1);
    if (before->y == after->y)
        ans += after->x - before->x;
    else
        ans += (self->columns - before->x) + after->x;
    return ans;
}
...
iteration_data(const Selection *sel, IterationData *ans, unsigned x_limit, int min_y, unsigned add_scrolled_by)
{
    ...
    // empty selection
    if (start->x == end->x && start_y == end_y && start->in_left_half_of_cell == end->in_left_half_of_cell)
        return;

    if (sel->rectangle_select)
    {
        // empty selection
        if (start->x == end->x && (!start->in_left_half_of_cell || end->in_left_half_of_cell))
            return;
...
xrange_for_iteration_with_multicells(const IterationData *idata, const int y, const Line *line)
{
    ...
    if (ans.x_limit > ans.x)
    {
        CPUCell *c;
        index_type ml;
        if (ans.x && (c = &line->cpu_cells[ans.x])->is_multicell && c->x)
            ans.x = ans.x > c->x ? ans.x - c->x : 0;
        if (ans.x_limit < line->xnum && (c = &line->cpu_cells[ans.x_limit - 1])->is_multicell && c->x + 1u < (ml = mcd_x_limit(c)))
        {
            ans.x_limit += ml - 1 - c->x;
            if (ans.x_limit > line->xnum)
                ans.x_limit = line->xnum;
        }
    }
    return ans;
}
...
apply_selection(Screen *self, uint8_t *data, Selection *s, uint8_t set_mask, int extra_leading_rows, int scrolled_by)
{
    ...
    for (int y = y_min; y < y_limit; y++)
    {
        if (self->paused_rendering.expires_at)
        {
            ...
            if (paused_y < 0 || paused_y >= (int)self->lines)
                continue;
        ...
        for (index_type x = xr.x; x < xr.x_limit; x++)
        {
            ...
            {
                for (int ym = MAX(0, y_in_data - c->y); ym < y_in_data; ym++)
...
bool screen_has_selection(Screen *self)
{
    IterationData idata;
    for (size_t i = 0; i < self->selections.count; i++)
    {
        ...
        if (!is_selection_empty(s))
        {
            ...
            if (!iteration_data_is_empty(self, &idata))
                return true;
        }
    }
    return false;
...
void screen_apply_selection(Screen *self, void *address_, size_t size)
{
    ...
    for (size_t i = 0; i < sel->count; i++)
        apply_selection(self, address, sel->items + i, 1, offset, scrolled_by);
    ...
    for (size_t i = 0; i < sel->count; i++)
    {
        ...
        if (OPT(underline_hyperlinks) == UNDERLINE_NEVER && s->is_hyperlink)
            continue;
    ...
    {
        if (ec->locations[i].cell < size)
            address[ec->locations[i].cell] |= (ec->locations[i].shape & 7) << 2;
    }
    ec->dirty = false;
}
...
flag_selection_to_extract_text(Screen *self, const Selection *s, int *miny, int *y_limit)
{
    ...
    *y_limit = MIN(idata.y_limit, (int)self->lines);
    ...
    while (*y_limit > *miny)
    {
        ...
        {
            if (temp.cpu_cells[x].temp_flag && temp.cpu_cells[x].ch_and_idx && (!temp.cpu_cells[x].is_multicell || !temp.cpu_cells[x].y))
                return;
        }
        (*y_limit)--;
    }
...
text_for_range(Screen *self, const Selection *sel, bool insert_newlines, bool strip_trailing_whitespace)
{
    ...
    for (int i = 0, y = min_y; y < y_limit; y++, i++)
    {
        ...
        while (x_limit && !line->cpu_cells[x_limit - 1].temp_flag)
            x_limit--;
        while (x_start < x_limit && !line->cpu_cells[x_start].temp_flag)
            x_start++;
        ...
        else
        {
            while (x_start < x_limit)
            {
                ...
                while (end < x_limit && line->cpu_cells[end].temp_flag)
                    end++;
                if (!unicode_in_range(line, x_start, end, true, add_trailing_newline, false, !is_first_line, &self->as_ansi_buf))
                    return PyErr_NoMemory();
    ...
    for (int i = 0, y = min_y; y < y_limit && i < PyTuple_GET_SIZE(ans) - 1; y++, i++)
    {
        ...
        else
        {
            ...
            while (x_start < x_limit)
            {
                ...
                if (line_as_ansi(line, &s, x_start, end, prefix_char, !is_first_line))
                    has_escape_codes = true;
    ...
    for (int y = idata.y; y < idata.y_limit && y < (int)self->lines; y++)
    {
        ...
        for (index_type x = xr.x; x < xr.x_limit; x++)
        {
            if (line->cpu_cells[x].hyperlink_id)
                return line->cpu_cells[x].hyperlink_id;
        }
    }
    return 0;
}
...
get_last_hostname_char_pos(Line *line, index_type url_start)
{
    ...
    while (url_start < line->xnum)
    {
        ...
        if (pos >= line->xnum)
            return line->xnum;
        if (++slash_count > 2)
            return prev_char_pos(line, pos, 1);
...
extend_url(Screen *screen, Line *line, index_type *x, index_type *y, char_type sentinel, bool newlines_allowed, index_type last_hostname_char_pos, index_type scale)
{
    ...
    while (count++ < 20)
    {
        ...
        if (next_char_pos(line, *x, 1) < line->xnum || (!newlines_allowed && has_newline))
            break;
        ...
        if (line)
        {
            ...
            if (next_line_starts_with_url_chars && has_newline && !newlines_allowed)
                next_line_starts_with_url_chars = false;
            if (sentinel && next_line_starts_with_url_chars && cell_is_char(line->cpu_cells, sentinel))
                next_line_starts_with_url_chars = false;
        }
    ...
    if (sentinel && *x == 0 && *y > orig_y)
    {
        ...
        if (line && cell_is_char(line->cpu_cells, sentinel))
        {
            *y -= scale;
            *x = line->xnum - 1;
            if (line->cpu_cells[*x].is_multicell)
                *x -= line->cpu_cells[*x].x;
        }
    }
}
...
int screen_detect_url(Screen *screen, unsigned int x, unsigned int y)
{
    ...
    if (!line || x >= screen->columns)
        return 0;
    if (line->cpu_cells[x].is_multicell && line->cpu_cells[x].scale > 1 && line->cpu_cells[x].y)
    {
        if (line->cpu_cells[x].y > y)
            return 0;
        y -= line->cpu_cells[x].y;
        line = screen_visual_line(screen, y);
    }
    if (line->cpu_cells[x].is_multicell && line->cpu_cells[x].x)
        x = x > line->cpu_cells[x].x ? x - line->cpu_cells[x].x : 0;
    ...
    if (url_start < line->xnum)
    {
        ...
        if (y + scale < screen->lines)
        {
            ...
            if (next_line_starts_with_url_chars && !newlines_allowed && !visual_line_is_continued(screen, y + scale))
                next_line_starts_with_url_chars = false;
        }
...
{
    if (!self->overlay_line.overlay_text)
        return;
    ...
    if (self->overlay_line.xstart < xstart)
        xstart = self->overlay_line.xstart;
...
#define WRAP1B(name, defval)                            \
    ...
    {                                                   \
        unsigned int v = defval;                        \
        int b = false;                                  \
        if (!PyArg_ParseTuple(args, "|Ip", &v, &b))     \
            return NULL;                                \
...
#define WRAP2B(name)                                    \
    ...
    {                                                   \
        unsigned int a, b;                              \
        int p;                                          \
        if (!PyArg_ParseTuple(args, "IIp", &a, &b, &p)) \
            return NULL;                                \
...
find_cmd_output(Screen *self, OutputOffset *oo, index_type start_screen_y, unsigned int scrolled_by, int direction, bool on_screen_only)
{
    ...
    if (direction >= 0)
    {
        ...
        {
            if (on_screen_only && !found_output && y2 > screen_limit)
                break;
...
erase_last_command(Screen *self, PyObject *args UNUSED)
{
    ...
    if (!found)
        Py_RETURN_FALSE; // nothing above the current prompt to erase
    const int start = y;
...
cmd_output(Screen *self, PyObject *args)
{
    ...
    if (!which_args || !as_text_args)
        return NULL;
    if (!PyArg_ParseTuple(which_args, "I", &which))
        return NULL;
    if (self->linebuf != self->main_linebuf)
        Py_RETURN_NONE;
    ...
    { // last non-empty output
        ...
        while (!found && !reached_upper_limit)
        {
            ...
            if (!line || (line->attrs.prompt_kind == OUTPUT_START))
            {
                ...
                {
                    if (!found_content)
                        found_content = !line_is_empty(line);
                    num_lines++;
                    y2++;
                }
                if (found_content)
    ...
    if (oo.reached_upper_limit && self->linebuf == self->main_linebuf && OPT(scrollback_pager_history_size) > 0)
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}

bool screen_set_last_visited_prompt(Screen *self, index_type y)
{
    if (y >= self->lines)
        return false;
...
bool screen_select_cmd_output(Screen *self, index_type y)
{
    if (y >= self->lines)
        return false;
    OutputOffset oo = {.screen = self};
    if (!find_cmd_output(self, &oo, y, self->scrolled_by, 0, true))
        return false;
...
screen_truncate_point_for_length(PyObject UNUSED *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "UI|I", &str, &num_cells, &start_pos))
        return NULL;
...
_select_graphic_rendition(Screen *self, PyObject *args)
{
    ...
    {
        params[i] = PyLong_AsLong(PyTuple_GET_ITEM(args, i));
    }
...
set_progress(Screen *self, PyObject *a)
{
    ...
    if (!PyArg_ParseTuple(a, "II", &state, &percent))
        return NULL;
...
static PyObject *scroll_until_cursor_prompt(Screen *self, PyObject *args)
{
    int b = false;
...
{
    if (self->cursor_render_info.render_even_when_unfocused)
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
update_selection(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "II|ppp", &x, &y, &in_left_half_of_cell, &ended, &nearest))
        return NULL;
...
WRAP0x(index)
    WRAP0(reverse_index) static PyObject *reset(Screen *self, PyObject *args)
{
    int hard = true;
    if (!PyArg_ParseTuple(args, "|p", &hard))
        return NULL;
    if (hard)
        screen_reset(self);
    else
        screen_soft_reset(self);
    Py_RETURN_NONE;
}
WRAP0(set_tab_stop)
...
start_selection(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "II|pip", &x, &y, &rectangle_select, &extend_mode, &in_left_half_of_cell))
        return NULL;
...
{
    if (self->selections.count && self->selections.items[0].rectangle_select)
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
text_for_selection(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|pp", &ansi, &strip_trailing_whitespace))
        return NULL;
...
screen_selection_range_for_line_(Line *line, index_type *start, index_type *end)
{
    ...
    while (xlimit > 0 && cell_is_blank(line->cpu_cells + xlimit - 1))
        xlimit--;
    while (xstart < xlimit && cell_is_blank(line->cpu_cells + xstart))
        xstart++;
    *start = xstart;
...
is_char_ok_for_word_extension(Line *line, index_type x, bool forward)
{
    ...
    if (char_props_for(ch).is_word_char || is_opt_word_char(ch, forward))
        return true;
...
bool screen_selection_range_for_word(Screen *self, const index_type x, const index_type y, index_type *y1, index_type *y2, index_type *s, index_type *e, bool initial_selection)
{
...
#define is_ok(x, forward) is_char_ok_for_word_extension(line, x, forward)
    ...
    while (true)
    {
        while (start > 0 && is_ok(start - 1, false))
            start--;
        if (start > 0 || !visual_line_is_continued(self, y) || *y1 == 0)
            break;
        line = visual_line_(self, *y1 - 1);
        if (!is_ok(self->columns - 1, false))
            break;
        (*y1)--;
    ...
    while (true)
    {
        while (end < self->columns - 1 && is_ok(end + 1, true))
            end++;
        if (end < self->columns - 1 || *y2 >= self->lines - 1)
            break;
        line = visual_line_(self, *y2 + 1);
        if (!visual_line_is_continued(self, *y2 + 1) || !is_ok(0, true))
            break;
        (*y2)++;
        end = 0;
    }
...
{
    if (!pixel_scroll_enabled(self))
        return false;
    if (!self->historybuf->count)
        return false;
    const double cell_height = (double)self->cell_size.height;
    if (cell_height <= 0.0 || delta_pixels == 0.0)
        return false;
...
screen_fractional_scroll(Screen *self, double amt)
{
    ...
    { // downwards
        ...
        else
        {
            ...
            if (pixels <= (int)self->pixel_scroll_offset_y)
                self->pixel_scroll_offset_y -= pixels;
            else
            {
                self->pixel_scroll_offset_y = 0;
                if (self->scrolled_by)
...
{
    double y;
    if (PyFloat_Check(amt))
        y = PyFloat_AS_DOUBLE(amt);
...
scroll_to_prompt(Screen *self, PyObject *args)
{
    ...
    int scroll_offset = 0;
    if (!PyArg_ParseTuple(args, "|ii", &num_of_prompts, &scroll_offset))
        return NULL;
    if (screen_history_scroll_to_prompt(self, num_of_prompts, scroll_offset))
    {
        Py_RETURN_TRUE;
    }
    Py_RETURN_FALSE;
...
set_last_visited_prompt(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|I", &visual_y))
        return NULL;
    if (screen_set_last_visited_prompt(self, visual_y))
    {
        Py_RETURN_TRUE;
    }
    Py_RETURN_FALSE;
...
bool screen_is_selection_dirty(Screen *self)
{
    ...
    if (self->paused_rendering.expires_at)
        return false;
    if (self->scrolled_by != self->last_rendered.scrolled_by)
        return true;
    if (self->selections.last_rendered_count != self->selections.count || self->url_ranges.last_rendered_count != self->url_ranges.count || self->extra_cursors.dirty)
        return true;
    for (size_t i = 0; i < self->selections.count; i++)
    {
        iteration_data(self->selections.items + i, &q, self->columns, 0, self->scrolled_by);
        if (memcmp(&q, &self->selections.items[i].last_rendered, sizeof(IterationData)) != 0)
            return true;
    }
    for (size_t i = 0; i < self->url_ranges.count; i++)
    {
        iteration_data(self->url_ranges.items + i, &q, self->columns, 0, self->scrolled_by);
        if (memcmp(&q, &self->url_ranges.items[i].last_rendered, sizeof(IterationData)) != 0)
            return true;
    }
    return false;
}

void screen_start_selection(Screen *self, index_type x, index_type y, bool in_left_half_of_cell, bool rectangle_select, SelectionExtendMode extend_mode)
{
    screen_pause_rendering(self, false, 0);
...
void screen_mark_url(Screen *self, index_type start_x, index_type start_y, index_type end_x, index_type end_y)
{
    ...
    if (start_x || start_y || end_x || end_y)
        add_url_range(self, start_x, start_y, end_x, end_y, false);
}
...
mark_hyperlinks_in_line(Screen *self, Line *line, hyperlink_id_type id, index_type y, bool *found_nonzero_multiline)
{
    ...
    *found_nonzero_multiline = false;
    ...
    if (in_range)
        add_url_range(self, start, y, self->columns - 1, y, true);
    return found;
}
...
screen_mark_hyperlink(Screen *self, index_type x, index_type y)
{
    ...
    do
    {
        if (mark_hyperlinks_in_line(self, line, id, ypos, &found_nonzero_multiline) || found_nonzero_multiline)
            last_marked_line = ypos;
        if (ypos == 0)
            break;
        ypos--;
    ...
    {
        line = screen_visual_line(self, ypos);
        if (mark_hyperlinks_in_line(self, line, id, ypos, &found_nonzero_multiline))
            last_marked_line = ypos;
        ypos++;
    }
    if (self->url_ranges.count > 1)
        sort_ranges(self, &self->url_ranges);
    return id;
}
...
continue_line_upwards(Screen *self, index_type top_line, SelectionBoundary *start, SelectionBoundary *end)
{
    ...
    {
        if (!screen_selection_range_for_line(self, top_line - 1, &start->x, &end->x))
            break;
        top_line--;
    }
    return top_line;
...
continue_line_downwards(Screen *self, index_type bottom_line, SelectionBoundary *start, SelectionBoundary *end)
{
    ...
    {
        if (!screen_selection_range_for_line(self, bottom_line + 1, &start->x, &end->x))
            break;
        bottom_line++;
    }
    return bottom_line;
...
continue_word_upwards_scrollback(Screen *self, int range_y, index_type *start_x)
{
    ...
    {
        if (!is_char_ok_for_word_extension(line, self->columns - 1, false))
            break;
        range_y--;
        num_in_scrollback++;
        index_type s = self->columns - 1;
        while (s > 0 && is_char_ok_for_word_extension(line, s - 1, false))
            s--;
        *start_x = s;
    }
    return num_in_scrollback;
...
continue_word_downwards_offscreen(Screen *self, int range_y, index_type *end_x)
{
    ...
    {
        if (!is_char_ok_for_word_extension(line, 0, true))
            break;
        range_y++;
        num_offscreen++;
        index_type e = 0;
        while (e < self->columns - 1 && is_char_ok_for_word_extension(line, e + 1, true))
            e++;
        *end_x = e;
    }
    return num_offscreen;
...
clamp_selection_input_to_multicell(Screen *self, const Selection *s, index_type x, index_type y, bool in_left_half_of_cell)
{
    ...
    if (abs_y == abs_start_y)
        return delta;
    Line *line = checked_range_line(self, abs_start_y);
    CPUCell *start, *current;
    if (!line || s->start.x >= line->xnum || !(start = &line->cpu_cells[s->start.x])->is_multicell || start->scale < 2)
        return delta;
    int abs_start_top = abs_start_y - start->y;
    line = checked_range_line(self, abs_y);
    if (x > s->start.x && in_left_half_of_cell)
        x--;
    else if (x < s->start.x && !in_left_half_of_cell)
        x++;
    if (!line || x >= line->xnum)
        return delta;
    current = line->cpu_cells + x;
    if (!current->is_multicell)
        return delta;
    int abs_current_top = abs_y - current->y;
    if (current->scale == start->scale && current->subscale_n == start->subscale_n && current->subscale_d == start->subscale_d && abs_current_top == abs_start_top)
...
#define set_abs(which, initializer, scrolled_by) \
        ...
        if (self->selections.extend_mode == EXTEND_LINE || self->selections.extend_mode == EXTEND_LINE_FROM_BEGIN || self->selections.extend_mode == EXTEND_LINE_FROM_POINT || self->selections.extend_mode == EXTEND_WORD_AND_LINE_FROM_POINT)
        {
            if (abs_start.y == abs_end.y)
            {
                if (abs_current_input.y == abs_start.y)
                    start_is_nearer = selection_boundary_less_than(&abs_start, &abs_end) ? (abs_current_input.x <= abs_start.x) : (abs_current_input.x <= abs_end.x);
                else
                    start_is_nearer = selection_boundary_less_than(&abs_start, &abs_end) ? (abs_current_input.y > abs_start.y) : (abs_current_input.y < abs_end.y);
            }
            else
            {
            ...
            start_is_nearer = num_cells_between_selection_boundaries(self, &abs_start, &abs_current_input) < num_cells_between_selection_boundaries(self, &abs_end, &abs_current_input);
        if (start_is_nearer)
            s->adjusting_start = true;
    }
    else if (!upd.start_extended_selection && self->selections.extend_mode != EXTEND_CELL)
    {
        ...
        if (self->selections.extend_mode == EXTEND_WORD)
        {
            ...
            {
                if (abs_start.y != abs_initial_start.y)
                    s->adjusting_start = true;
                else if (abs_end.y != abs_initial_start.y)
            ...
            else
            {
                if (s->adjusting_start)
                    ...
                    s->adjusting_start = selection_boundary_less_than(&abs_current_input, &abs_initial_start);
            }
        }
        else
        {
...
#undef set_abs
    ...
    if (s->adjusting_start)
        adjusted_boundary_is_before = selection_boundary_less_than(&abs_start, &abs_end);
    else
    {
        adjusted_boundary_is_before = selection_boundary_less_than(&abs_end, &abs_start);
    }
    ...
    case EXTEND_WORD:
    {
        ...
        else if (word_found_at_cursor)
        {
            ...
            if (s->adjusting_start || adjust_both_ends)
                s->start_scrolled_by = self->scrolled_by;
            if (!s->adjusting_start || adjust_both_ends)
...
void screen_update_selection(Screen *self, index_type x, index_type y, bool in_left_half_of_cell, SelectionUpdate upd)
{
...
pause_rendering(Screen *self, PyObject *args)
{
    int msec = 100;
    int pause = 1;
    if (!PyArg_ParseTuple(args, "|pi", &pause, &msec))
        return NULL;
    if (screen_pause_rendering(self, pause, msec))
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
send_escape_code_to_child(Screen *self, PyObject *args)
{
    int code;
    PyObject *O;
    if (!PyArg_ParseTuple(args, "iO", &code, &O))
        return NULL;
    bool written = false;
    if (PyBytes_Check(O))
        written = write_escape_code_to_child(self, code, PyBytes_AS_STRING(O));
    else if (PyUnicode_Check(O))
    {
        const char *t = PyUnicode_AsUTF8(O);
        if (t)
            written = write_escape_code_to_child(self, code, t);
        else
            return NULL;
    }
    else if (PyTuple_Check(O))
        written = write_escape_code_to_child_python(self, code, O);
    else
        PyErr_SetString(PyExc_TypeError, "escape code must be str, bytes or tuple");
    if (PyErr_Occurred())
        return NULL;
    if (written)
    {
...
set_marker(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O", &marker))
        return NULL;
    if (!marker)
    {
        if (self->marker)
...
scroll_to_next_mark(Screen *self, PyObject *args)
{
    int backwards = 1;
    unsigned int mark = 0;
    if (!PyArg_ParseTuple(args, "|Ip", &mark, &backwards))
        return NULL;
    if (!screen_has_marker(self) || self->linebuf == self->alt_linebuf)
        Py_RETURN_FALSE;
    if (backwards)
    {
...
focus_changed(Screen *self, PyObject *has_focus_)
{
    ...
    if (has_focus != previous)
    {
        ...
        if (has_focus)
            self->has_activity_since_last_focus = false;
        else if (screen_is_overlay_active(self))
            deactivate_overlay_line(self);
...
{
    if (self->has_activity_since_last_focus)
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
reverse_scroll(Screen *self, PyObject *args)
{
    ...
    unsigned int amt;
    if (!PyArg_ParseTuple(args, "I|p", &amt, &fill_from_scrollback))
        return NULL;
...
{
    if (self->linebuf != self->main_linebuf || !self->historybuf->count)
        Py_RETURN_NONE;
    int q = screen_cursor_at_a_shell_prompt(self);
...
#define call_string(s)                                            \
    ...
    case UNKNOWN_PROMPT_KIND:
        break;
    case PROMPT_START:
        call_string("\x1b[32mprompt \x1b[39m");
        break;
    case SECONDARY_PROMPT:
        call_string("\x1b[32msecondary_prompt \x1b[39m");
        break;
    case OUTPUT_START:
        call_string("\x1b[33moutput \x1b[39m");
        break;
    }
    if (range_line_is_continued(self, y))
        call_string("continued ");
    if (line->attrs.has_dirty_text)
        call_string("dirty ");
    call_string("\n");
...
dump_lines_with_attrs(Screen *self, PyObject *args)
{
    PyObject *accum;
    int which_screen = -1;
    if (!PyArg_ParseTuple(args, "O|i", &accum, &which_screen))
        return NULL;
    LineBuf *orig = self->linebuf;
    switch (which_screen)
    {
    ...
    while (y < (int)self->lines && !PyErr_Occurred())
        dump_line_with_attrs(self, y++, accum);
    self->linebuf = orig;
    if (PyErr_Occurred())
        return NULL;
    Py_RETURN_NONE;
...
static PyObject *
raw_cursor_at_prompt_state(Screen *self, PyObject *args UNUSED)
{
    return PyLong_FromLong(screen_cursor_at_a_shell_prompt(self));
}
...
{
    if (screen_mark_potential_url_drag(self))
        Py_RETURN_TRUE;
    Py_RETURN_FALSE;
}
...
test_draw_overlay_line(Screen *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "UII", &text, &xstart, &ynum))
        return NULL;
...
test_commit_write_buffer(Screen *screen, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "y*y*", &srcbuf, &destbuf))
        return NULL;
...
test_parse_written_data(Screen *screen, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "|O", &pd.dump_callback))
        return NULL;
    if (pd.dump_callback && pd.dump_callback != Py_None)
        parse_worker_dump(screen, &pd, true);
    else
...
cpu_cell_as_dict(CPUCell *c, TextCache *tc, ListOfChars *lc, HYPERLINK_POOL_HANDLE h)
{
    ...
    if ((c->is_multicell && (c->x + c->y)) || (lc->count == 1 && lc->chars[0] == 0))
        lc->count = 0;
...
{
    int y, x = -1;
    if (!PyArg_ParseTuple(args, "i|i", &y, &x))
        return NULL;
    if (y >= (int)self->lines)
    {
    ...
    CPUCell *cells;
    if (y >= 0)
        cells = linebuf_cpu_cells_for_line(self->linebuf, y);
    else
    {
            ...
            MND(dump_lines_with_attrs, METH_VARARGS)
                MND(cpu_cells, METH_VARARGS)
                    MND(cursor_at_prompt, METH_NOARGS)
                        MND(raw_cursor_at_prompt_state, METH_NOARGS){"visual_line", (PyCFunction)pyvisual_line, METH_VARARGS, ""},
                                                                                                                                ...
                                                                                                                                MND(update_only_line_graphics_data, METH_NOARGS)
                                                                                                                                    MND(bell, METH_NOARGS)
                                                                                                                                        MND(mark_potential_url_drag, METH_NOARGS)
                                                                                                                                            MND(current_selections, METH_NOARGS){"select_graphic_rendition", (PyCFunction)_select_graphic_rendition, METH_VARARGS, ""},
