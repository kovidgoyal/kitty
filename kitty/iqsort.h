...
#define QSORT(QSORT_TYPE, QSORT_BASE, QSORT_NELT, QSORT_LT)                                          \
    {                                                                                                \
        ...
        if (_elems > _QSORT_MAX_THRESH) {                                                            \
            ...
            while (_QSORT_STACK_NOT_EMPTY) {                                                         \
                ...
                QSORT_TYPE *_mid = _lo + ((_hi - _lo) >> 1);                                         \
                                                                                                     \
                if (QSORT_LT(_mid, _lo)) _QSORT_SWAP(_mid, _lo, _hold);                              \
                ...
                do {                                                                                 \
                    while (QSORT_LT(_left_ptr, _mid)) ++_left_ptr;                                   \
                                                                                                     \
                    while (QSORT_LT(_mid, _right_ptr)) --_right_ptr;                                 \
                                                                                                     \
                    if (_left_ptr < _right_ptr) {                                                    \
                        _QSORT_SWAP(_left_ptr, _right_ptr, _hold);                                   \
                        if (_mid == _left_ptr) _mid = _right_ptr;                                    \
                        else if (_mid == _right_ptr) _mid = _left_ptr;                               \
                        ++_left_ptr;                                                                 \
                        --_right_ptr;                                                                \
        ...
        {                                                                                            \
            ...
            if (_thresh > _end_ptr) _thresh = _end_ptr;                                              \
                                                                                                     \
            ...
            for (_run_ptr = _tmp_ptr + 1; _run_ptr <= _thresh; ++_run_ptr)                           \
                if (QSORT_LT(_run_ptr, _tmp_ptr)) _tmp_ptr = _run_ptr;                               \
                                                                                                     \
            if (_tmp_ptr != _base) _QSORT_SWAP(_tmp_ptr, _base, _hold);                              \
                                                                                                     \
            ...
            while (++_run_ptr <= _end_ptr) {                                                         \
                _tmp_ptr = _run_ptr - 1;                                                             \
                while (QSORT_LT(_run_ptr, _tmp_ptr)) --_tmp_ptr;                                     \
                                                                                                     \
                ++_tmp_ptr;                                                                          \
                if (_tmp_ptr != _run_ptr) {                                                          \
                    ...
                    while (--_trav >= _run_ptr) {                                                    \
                        ...
                        _hold = *_trav;                                                              \
                                                                                                     \
                        for (_hi = _lo = _trav; --_lo >= _tmp_ptr; _hi = _lo) *_hi = *_lo;           \
                        *_hi = _hold;                                                                \
                    }                                                                                \
                }                                                                                    \
            }                                                                                        \
