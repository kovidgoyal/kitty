...
def parse_test_data() -> None:
    ...
    grapheme_segmentation_tests.append({'data': (' ', '\xad', ' '), 'comment': '÷ [0.2] SPACE (Other) ÷ [0.4] SOFT HYPHEN ÷ [999.0] SPACE (Other) ÷ [0.3]'})
    grapheme_segmentation_tests.append(
        {
            'data': ('\U0001f468\u200d\U0001f469\u200d\U0001f467\u200d\U0001f466',),
            'comment': '÷ [0.2] MAN × [9.0] ZERO WIDTH JOINER × [11.0] WOMAN × [9.0] ZERO WIDTH JOINER'
            ' × [11.0] GIRL × [9.0] ZERO WIDTH JOINER × [11.0] BOY ÷ [0.3]',
        }
    )
...
def gen_rowcolumn_diacritics() -> None:
    ...
    with create_header('kitty/rowcolumn-diacritics.c') as p, create_header(go_file, include_data_types=False) as g:
        ...
        def print_range() -> None:
            ...
            p('\t\treturn code - ' + hex(range_start) + ' + ' + str(range_start_num) + ';')
...
def bitfield_from_int(fields: dict[str, int], x: int, int_to_str: dict[str, tuple[str, ...]]) -> dict[str, str | bool]:
...
def bitfield_as_int(fields: dict[str, int], vals: Sequence[bool | str], str_maps: dict[str, dict[str, int]]) -> int:
    # first field is least significant, last field is most significant
    ans = shift = 0
...
class GraphemeSegmentationState(NamedTuple):
    ...
    def add_to_current_cell(self, p: GraphemeSegmentationProps) -> 'GraphemeSegmentationResult':
        ...
        if self.grapheme_break == 'AtStart':
            ...
            elif (prev == 'L' and prop in ('L', 'V', 'LV', 'LVT')) or (prev in ('LV', 'V') and prop in ('V', 'T')) or (prev in ('LVT', 'T') and prop == 'T'):
                add_to_cell = True
        ...
        incb_consonant_extended_linker_extended = incb_consonant_extended_linker or (self.incb_consonant_extended_linker_extended and incb in linker_or_extend)
        incb_consonant_extended = incb == 'Consonant' or (self.incb_consonant_extended and incb in linker_or_extend)
        ...
        return GraphemeSegmentationResult(
            GraphemeSegmentationState(
                grapheme_break=prop,
                incb_consonant_extended=incb_consonant_extended,
                ...
                emoji_modifier_sequence_before_last_char=emoji_modifier_sequence_before_last_char,
            ),
            add_to_cell,
        )
...
def split_into_graphemes_with_table(
    props: Sequence['GraphemeSegmentationProps'],
    table: Sequence['GraphemeSegmentationResult'],
    text: str,
) -> Iterator[str]:
...
func (s CharProps) GraphemeSegmentationProperty() {base_type} {{
    ...
    def c_declaration(cls) -> str:
        ...
        alternate = {'grapheme_segmentation_property': sum(int(cls._field_defaults[f]) for f in GraphemeSegmentationProps._fields)}
...
def gen_char_props() -> None:
    ...
    prop_array = tuple(
        CharProps(
            width=width_map.get(ch, 1),
            grapheme_break=gs_map.get(ch, 'None'),
            indic_conjunct_break=icb_map.get(ch, 'None'),
            is_invalid=ch in invalid,
            is_non_rendered=ch in non_rendered,
            is_emoji=ch in all_emoji,
            is_symbol=ch in all_symbols,
            is_extended_pictographic=ch in extended_pictographic,
            is_emoji_presentation_base=ch in emoji_presentation_bases,
            is_combining_char=ch in marks,
            category=cat_map.get(ch, 'Cn'),
            is_word_char=ch in is_word_char,
    ...
    gsprops = tuple(
        GraphemeSegmentationProps(
            grapheme_break=x.grapheme_break, indic_conjunct_break=x.indic_conjunct_break, is_extended_pictographic=x.is_extended_pictographic
        )
        for x in prop_array
    )
