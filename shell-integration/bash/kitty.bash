...
if [[ -n "$KITTY_BASH_INJECT" ]]; then
    ...
    if [[ -n "$ksi_val" && "$ksi_val" != *no-sudo* && -n "$TERMINFO" && ! ( -r "/usr/share/terminfo/x/xterm-kitty" || -r "/usr/share/terminfo/78/xterm-kitty" ) ]]; then
        ...
        sudo() {
            ...
            builtin local skip_next="n"
            builtin local flags flag i
            for arg; do
                if [[ "$skip_next" == "y" ]]; then skip_next="n"; continue; fi
                [[ "$arg" == "--" ]] && builtin break  # end of options
                if [[ "$arg" == "--edit" || "$arg" == "--validate" ]]; then
                    is_sudoedit="y"; builtin break
                fi
                if [[ "$arg" == --* ]]; then
                    if [[ "$arg" != *=* ]]; then
                        case "$arg" in
                            --user|--group|--host|--chdir|--chroot|--role|--type|--command-timeout|--auth-type|--login-class|--prompt|--close-from|--other-user)
                                skip_next="y";;
                        esac
                    fi
                    continue
                ...
                [[ "$arg" != *=* ]] && builtin break  # command found
            done
