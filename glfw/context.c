...
_glfwRefreshContextAttribs(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig) {
    ...
    if (sscanf(version, "%d.%d.%d", &window->context.major, &window->context.minor, &window->context.revision) < 1) {
    ...
    if (window->context.client == GLFW_OPENGL_API) {
        ...
        if (window->context.major >= 3) {
            ...
            if (flags & GL_CONTEXT_FLAG_FORWARD_COMPATIBLE_BIT) window->context.forward = true;

            if (flags & GL_CONTEXT_FLAG_DEBUG_BIT) window->context.debug = true;
            ...
            if (flags & GL_CONTEXT_FLAG_NO_ERROR_BIT_KHR) window->context.noerror = true;
        }
...
glfwExtensionSupported(const char *extension) {
    ...
    if (window->context.major >= 3) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (strcmp(en, extension) == 0) return true;
        }
    } else {
