...
//
static const char *
getEGLErrorString(EGLint error) {
    switch (error) {
...
//
static int
getEGLConfigAttrib(EGLConfig config, int attrib) {
    int value;
...
static void
makeContextCurrentEGL(_GLFWwindow *window) {
    if (window) {
        ...
        if (!eglMakeCurrent(_glfw.egl.display, EGL_NO_SURFACE, EGL_NO_SURFACE, EGL_NO_CONTEXT)) {
            _glfwInputError(GLFW_PLATFORM_ERROR, "EGL: Failed to clear current context: %s", getEGLErrorString(eglGetError()));
            return;
        }
    }
...
static void
swapBuffersEGL(_GLFWwindow *window) {
    if (window != _glfwPlatformGetTls(&_glfw.contextSlot)) {
        _glfwInputError(GLFW_PLATFORM_ERROR, "EGL: The context must be current on the calling thread when swapping buffers");
        return;
    }
...
static void
swapIntervalEGL(int interval) {
    eglSwapInterval(_glfw.egl.display, interval);
}

static int
extensionSupportedEGL(const char *extension) {
...
static void
destroyContextEGL(_GLFWwindow *window) {
#if defined(_GLFW_X11)
...
_glfwInitEGL(void) {
    ...
    if (_glfw.egl.handle) return true;
    ...
    if (!_glfw.egl.GetConfigAttrib || !_glfw.egl.GetConfigs || !_glfw.egl.ChooseConfig || !_glfw.egl.GetDisplay || !_glfw.egl.GetError ||
        !_glfw.egl.Initialize || !_glfw.egl.Terminate || !_glfw.egl.BindAPI || !_glfw.egl.CreateContext || !_glfw.egl.DestroySurface ||
        !_glfw.egl.DestroyContext || !_glfw.egl.CreateWindowSurface || !_glfw.egl.MakeCurrent || !_glfw.egl.SwapBuffers || !_glfw.egl.SwapInterval ||
    ...
    if (extensions && eglGetError() == EGL_SUCCESS) _glfw.egl.EXT_client_extensions = true;
    ...
    if (_glfw.egl.EXT_platform_base) {
        ...
        _glfw.egl.CreatePlatformWindowSurfaceEXT = (PFNEGLCREATEPLATFORMWINDOWSURFACEEXTPROC)eglGetProcAddress("eglCreatePlatformWindowSurfaceEXT");
    }

    _glfw.egl.platform = _glfwPlatformGetEGLPlatform(&attribs);
    _glfw.egl.display = EGL_NO_DISPLAY;
    if (_glfw.egl.platform) { _glfw.egl.display = eglGetPlatformDisplayEXT(_glfw.egl.platform, _glfwPlatformGetEGLNativeDisplay(), attribs); }
    free(attribs);

    if (_glfw.egl.display == EGL_NO_DISPLAY) _glfw.egl.display = eglGetDisplay(_glfwPlatformGetEGLNativeDisplay());


    EGLint egl_err;
    if (_glfw.egl.display == EGL_NO_DISPLAY && (egl_err = eglGetError()) != EGL_SUCCESS) {
        _glfwInputError(GLFW_API_UNAVAILABLE, "EGL: Failed to get EGL display: %s", getEGLErrorString(egl_err));
...
//
bool
_glfwCreateContextEGL(_GLFWwindow *window, const _GLFWctxconfig *ctxconfig, const _GLFWfbconfig *fbconfig) {
    EGLint attribs[40];
    EGLConfig config;
    ...
    if (_glfw.egl.KHR_create_context) {
        ...
        if (ctxconfig->noerror) {
            if (_glfw.egl.KHR_create_context_no_error) setAttrib(EGL_CONTEXT_OPENGL_NO_ERROR_KHR, true);
        }
    ...
    if (!eglGetConfigAttrib(_glfw.egl.display, config, a, &a)) {
        ...
        if (a > 0) { _glfwInputError(GLFW_VERSION_UNAVAILABLE, "EGL: non-blocking swap buffers not available, minimum swap interval is: %d", a); }
    }
...
#if defined(_GLFW_X11)
bool
_glfwChooseVisualEGL(const _GLFWwndconfig *wndconfig UNUSED, const _GLFWctxconfig *ctxconfig, const _GLFWfbconfig *fbconfig, Visual **visual, int *depth) {
    XVisualInfo *result;
    XVisualInfo desired;
    EGLConfig native;
...
GLFWAPI EGLContext
glfwGetEGLContext(GLFWwindow *handle) {
