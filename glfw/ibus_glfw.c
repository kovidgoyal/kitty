...
#define debug(...) \
    if (_glfw.hints.init.debugKeyboard) printf(__VA_ARGS__);
...
static inline bool test_env_var(const char *name, const char *val) {
    const char *q = getenv(name);
    return (q && strcmp(q, val) == 0) ? true : false;
}

static inline size_t GLFW_MIN(size_t a, size_t b) { return a < b ? a : b; }

static const char *get_ibus_text_from_message(DBusMessage *msg) {
...
static inline void send_text(const char *text, GLFWIMEState ime_state) {
...
static DBusHandlerResult message_handler(DBusConnection *conn UNUSED,
    ...
    (void)ibus;
    const char *text;
    switch (glfw_dbus_match_signal(msg, IBUS_INPUT_INTERFACE, "CommitText",
				   "UpdatePreeditText", "HidePreeditText",
				   "ShowPreeditText", NULL)) {
	case 0:
...
static inline const char *get_ibus_address_file_name(void) {
    const char *addr;
	...
	_glfwInputError(GLFW_PLATFORM_ERROR,
			"Could not get IBUS address file name as DISPLAY env "
			"var has no colon");
	free(display);
	return NULL;
    }
    *disp_num = 0;
	...
	if (!conf_env || !conf_env[0]) {
	    _glfwInputError(GLFW_PLATFORM_ERROR,
			    "Could not get IBUS address file name as no HOME "
			    "env var is set");
	    free(display);
	    return NULL;
	}
    ...
    snprintf(ans + offset, sizeof(ans) - offset, "/ibus/bus/%s-%s-%s", key,
	     host, disp_num);
    dbus_free(key);
    free(display);
    return ans;
...
static inline bool read_ibus_address(_GLFWIBUSData *ibus) {
    static char buf[1024];
    struct stat s;
    FILE *addr_file = fopen(ibus->address_file_name, "r");
    if (!addr_file) {
	_glfwInputError(GLFW_PLATFORM_ERROR,
			"Failed to open IBUS address file: %s with error: %s",
			ibus->address_file_name, strerror(errno));
	return false;
    }
    ...
    if (stat_result != 0) {
	_glfwInputError(GLFW_PLATFORM_ERROR,
			"Failed to stat IBUS address file: %s with error: %s",
			ibus->address_file_name, strerror(errno));
	return false;
    }
    ...
    _glfwInputError(GLFW_PLATFORM_ERROR, "Could not find IBUS_ADDRESS in %s",
		    ibus->address_file_name);
    return false;
}

static void input_context_created(DBusMessage *msg, const char *errmsg,
				  void *data) {
    if (errmsg) {
	_glfwInputError(GLFW_PLATFORM_ERROR,
			"IBUS: Failed to create input context with error: %s",
			errmsg);
	return;
    }
    const char *path = NULL;
    if (!glfw_dbus_get_args(msg, "Failed to get IBUS context path from reply",
    ...
    dbus_bus_add_match(
	ibus->conn,
	"type='signal',interface='org.freedesktop.IBus.InputContext'", NULL);
    DBusObjectPathVTable ibus_vtable = {.message_function = message_handler};
    dbus_connection_try_register_object_path(ibus->conn, ibus->input_ctx_path,
					     &ibus_vtable, ibus, NULL);
    enum Capabilities caps = IBUS_CAP_FOCUS | IBUS_CAP_PREEDIT_TEXT;
    if (!glfw_dbus_call_method_no_reply(
	    ibus->conn, IBUS_SERVICE, ibus->input_ctx_path,
	    IBUS_INPUT_INTERFACE, "SetCapabilities", DBUS_TYPE_UINT32, &caps,
	    DBUS_TYPE_INVALID))
	return;
    ibus->ok = true;
...
static bool setup_connection(_GLFWIBUSData *ibus) {
    ...
    debug("Connecting to IBUS daemon @ %s for IME input management\n",
	  ibus->address);
    ibus->conn = glfw_dbus_connect_to(
	ibus->address, "Failed to connect to the IBUS daemon, with error",
	"ibus", true);
    if (!ibus->conn) return false;
    free((void *)ibus->input_ctx_path);
    ibus->input_ctx_path = NULL;
    if (!glfw_dbus_call_method_with_reply(
	    ibus->conn, IBUS_SERVICE, IBUS_PATH, IBUS_INTERFACE,
	    "CreateInputContext", DBUS_TIMEOUT_USE_DEFAULT,
	    input_context_created, ibus, DBUS_TYPE_STRING, &client_name,
	    DBUS_TYPE_INVALID)) {
	return false;
    }
    return true;
...
static bool check_connection(_GLFWIBUSData *ibus) {
    if (!ibus->inited) return false;
    if (ibus->conn && dbus_connection_get_is_connected(ibus->conn)) {
	return ibus->ok;
    }
    struct stat s;
    if (stat(ibus->address_file_name, &s) != 0 ||
	s.st_mtime != ibus->address_file_mtime) {
	if (!read_ibus_address(ibus)) return false;
	setup_connection(ibus);
    }
    return false;
...
static void simple_message(_GLFWIBUSData *ibus, const char *method) {
    if (check_connection(ibus)) {
	glfw_dbus_call_method_no_reply(
	    ibus->conn, IBUS_SERVICE, ibus->input_ctx_path,
	    IBUS_INPUT_INTERFACE, method, DBUS_TYPE_INVALID);
    }
}
...
void glfw_ibus_set_cursor_geometry(_GLFWIBUSData *ibus, int x, int y, int w,
				   int h) {
    if (check_connection(ibus)) {
	glfw_dbus_call_method_no_reply(
	    ibus->conn, IBUS_SERVICE, ibus->input_ctx_path,
	    IBUS_INPUT_INTERFACE, "SetCursorLocation", DBUS_TYPE_INT32, &x,
	    DBUS_TYPE_INT32, &y, DBUS_TYPE_INT32, &w, DBUS_TYPE_INT32, &h,
	    DBUS_TYPE_INVALID);
    }
}
...
static inline uint32_t ibus_key_state(unsigned int glfw_modifiers, int action) {
...
static void key_event_processed(DBusMessage *msg, const char *errmsg,
				void *data) {
    uint32_t handled = 0;
    ...
    if (errmsg) {
	_glfwInputError(GLFW_PLATFORM_ERROR,
			"IBUS: Failed to process key with error: %s", errmsg);
	failed = true;
    } else {
	glfw_dbus_get_args(msg, "Failed to get IBUS handled key from reply",
			   DBUS_TYPE_BOOLEAN, &handled, DBUS_TYPE_INVALID);
	debug("IBUS processed native_key: 0x%x release: %d handled: %u\n",
	      ev->glfw_ev.native_key, is_release, handled);
    }
    glfw_xkb_key_from_ime((_GLFWIMEKeyEvent *)ev, _GLFW_IME_MODULE_IBUS,
			  handled ? true : false, failed);
    free(ev);
}
...
bool ibus_process_key(const _GLFWIBUSKeyEvent *ev_, _GLFWIBUSData *ibus) {
    ...
    if (ev->glfw_ev.text)
	strncpy(ev->__embedded_text, ev->glfw_ev.text,
		sizeof(ev->__embedded_text) - 1);
    ev->glfw_ev.text = NULL;
    uint32_t state = ibus_key_state(ev->glfw_ev.mods, ev->glfw_ev.action);
    if (!glfw_dbus_call_method_with_reply(
	    ibus->conn, IBUS_SERVICE, ibus->input_ctx_path,
	    IBUS_INPUT_INTERFACE, "ProcessKeyEvent", 3000, key_event_processed,
	    ev, DBUS_TYPE_UINT32, &ev->ibus_keysym, DBUS_TYPE_UINT32,
	    &ev->ibus_keycode, DBUS_TYPE_UINT32, &state, DBUS_TYPE_INVALID)) {
	free(ev);
	return false;
    }
    return true;
