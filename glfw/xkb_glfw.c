...
glfw_key_for_sym(xkb_keysym_t key) {
    switch (key) {
        ...
        case XKB_KEY_ISO_Level5_Shift:
            return GLFW_FKEY_ISO_LEVEL5_SHIFT;
            /* end xkb to glfw */
        default: return xkb_keysym_to_utf32(key);
    }
}
...
glfw_xkb_sym_for_key(uint32_t key) {
    switch (key) {
        ...
        case GLFW_FKEY_ISO_LEVEL5_SHIFT:
            ...
            /* end glfw to xkb */
        default: return utf32_to_keysym(key);
    }
}
...
#undef S
    debug(
        "Modifier indices alt: 0x%x super: 0x%x hyper: 0x%x meta: 0x%x numlock: 0x%x shift: 0x%x capslock: 0x%x\n",
        xkb->altIdx,
        xkb->superIdx,
        xkb->hyperIdx,
        xkb->metaIdx,
 ...
 */
static void
modifier_mapping_algorithm(struct xkb_keymap *keymap UNUSED, xkb_keycode_t key, void *data) {
    modifier_mapping_algorithm_t *algorithm = (modifier_mapping_algorithm_t *)data;
    if (algorithm->failed) return;
    ...
    if (changed_type & (XKB_STATE_MODS_DEPRESSED | XKB_STATE_MODS_LATCHED | XKB_STATE_MODS_LOCKED)) {
        xkb_mod_mask_t mods = xkb_state_serialize_mods(
            algorithm->state, algorithm->try_shift ? XKB_STATE_MODS_EFFECTIVE : (XKB_STATE_MODS_DEPRESSED | XKB_STATE_MODS_LATCHED | XKB_STATE_MODS_LOCKED));
...
#undef S2
        ...
        if (!algorithm->shift_keycode && (keysyms[0] == XKB_KEY_Shift_L || keysyms[0] == XKB_KEY_Shift_R)) algorithm->shift_keycode = key;
    ...
    if (algorithm->try_shift) { xkb_state_update_key(algorithm->state, algorithm->shift_keycode, XKB_KEY_UP); }
}

static int
local_modifier_mapping(_GLFWXKBData *xkb) {
    ...
    if (algorithm.state != NULL) {
        ...
        if (!algorithm.shift_keycode) algorithm.failed = 1;

        if (!(algorithm.shift && algorithm.control && algorithm.alt && algorithm.super && algorithm.meta && algorithm.hyper) && !algorithm.failed) {
...
#undef S
    ...
    if (algorithm.failed) debug("Wayland modifier autodetection algorithm failed; using defaults\n");
...
#undef S
    ...
    debug(
        "Modifier indices alt: 0x%x super: 0x%x hyper: 0x%x meta: 0x%x numlock: 0x%x shift: 0x%x capslock: 0x%x control: 0x%x\n",
        xkb->altIdx,
        xkb->superIdx,
        xkb->hyperIdx,
        xkb->metaIdx,
...
glfw_xkb_create_context(_GLFWXKBData *xkb) {
    ...
    if (!xkb->context) {
        _glfwInputError(GLFW_PLATFORM_ERROR, "Failed to initialize XKB context");
        return false;
    }
...
load_compose_tables(_GLFWXKBData *xkb) {
    ...
    if (!xkb->states.composeState) { _glfwInputError(GLFW_PLATFORM_ERROR, "Failed to create XKB compose state"); }
...
static unsigned int
update_one_modifier(XKBStateGroup *group, xkb_mod_mask_t mask, xkb_mod_index_t idx, unsigned int mod) {
    if (idx == XKB_MOD_INVALID) return 0;
    /* Optimization in the case of a single real modifier */
    if (mask && ((mask & (mask - 1)) == 0)) return (xkb_state_mod_index_is_active(group->state, idx, XKB_STATE_MODS_EFFECTIVE) == 1) ? mod : 0;
    /* Multiple real mods map to the same virtual mod */
    for (unsigned indx = 0; indx < 32 && mask; ++indx, mask >>= 1)
        if ((mask & 1) && xkb_state_mod_index_is_active(group->state, indx, XKB_STATE_MODS_EFFECTIVE) == 1) return mod;
    return 0;
}
...
glfw_xkb_compile_keymap(_GLFWXKBData *xkb, const char *map_str) {
    ...
    for (xkb_mod_index_t i = 0; i < xkb_keymap_num_mods(xkb->keymap) && j < capacity - 1; i++) {
        if (i != xkb->controlIdx && i != xkb->altIdx && i != xkb->shiftIdx && i != xkb->superIdx && i != xkb->hyperIdx && i != xkb->metaIdx &&
...
// Map a GLFW modifier bit to the xkb modifier index for this keymap.
static xkb_mod_index_t
xkb_idx_for_glfw_mod(_GLFWXKBData *xkb, int glfw_mod) {
    switch (glfw_mod) {
        case GLFW_MOD_SHIFT: return xkb->shiftIdx;
        case GLFW_MOD_CONTROL: return xkb->controlIdx;
        case GLFW_MOD_ALT: return xkb->altIdx;
        case GLFW_MOD_SUPER: return xkb->superIdx;
        case GLFW_MOD_HYPER: return xkb->hyperIdx;
        case GLFW_MOD_META: return xkb->metaIdx;
        default: return XKB_MOD_INVALID;
    }
}

// Apply remap_modifier to an xkb modifier mask, BEFORE the state is used to
// produce keysyms and text. Doing it here rather than on the finished event is
// what keeps the two consistent: a modifier that participates in text production
// (shift always; others depending on the layout) would otherwise report one
// modifier while carrying text generated by another.
static xkb_mod_mask_t
remap_xkb_mask(_GLFWXKBData *xkb, xkb_mod_mask_t mask) {
    if (!_glfw.modifierRemapMask) return mask;
    xkb_mod_mask_t consumed = 0, produced = 0;
    for (unsigned i = 0; i < GLFW_MOD_REMAP_SZ; i++) {
        const int dest = _glfw.modifierRemap[i];
        if (!dest) continue;
        const xkb_mod_index_t src_idx = xkb_idx_for_glfw_mod(xkb, 1 << i);
        if (src_idx == XKB_MOD_INVALID || !(mask & (1u << src_idx))) continue;
        consumed |= 1u << src_idx;
        for (unsigned j = 0; j < GLFW_MOD_REMAP_SZ; j++) {
            if (!(dest & (1 << j))) continue;
            const xkb_mod_index_t dst_idx = xkb_idx_for_glfw_mod(xkb, 1 << j);
            if (dst_idx != XKB_MOD_INVALID) produced |= 1u << dst_idx;
        }
    }
    return (mask & ~consumed) | produced;
}
...
glfw_xkb_update_modifiers(
    ...
    xkb->states.modifiers = 0;
    if (_glfw.modifierRemapMask) {
        const xkb_mod_mask_t before = depressed;
        depressed = remap_xkb_mask(xkb, depressed);
        latched = remap_xkb_mask(xkb, latched);
        locked = remap_xkb_mask(xkb, locked);
        if (before != depressed) debug("remap_modifier: xkb depressed mask 0x%x -> 0x%x\n", before, depressed);
    }
...
#define pr(x)                                                              \
    ...
    for (xkb_mod_index_t i = 0; i < xkb_keymap_num_mods(xkb->keymap); i++) {
        ...
        if (m & mods) {
            pr(xkb_keymap_mod_get_name(xkb->keymap, i));
            pr("+");
        }
    }
    if (p == s) {
...
glfw_xkb_update_ime_state(_GLFWwindow *w, _GLFWXKBData *xkb, const GLFWIMEUpdateEvent *ev) {
    ...
    int x = 0, y = 0;
    switch (ev->type) {
        case GLFW_IME_UPDATE_FOCUS: glfw_ibus_set_focused(&xkb->ibus, ev->focused); break;
...
glfw_xkb_key_from_ime(_GLFWIBUSKeyEvent *ev, bool handled_by_ime, bool failed) {
    ...
    debug(
        "From IBUS: native_key: 0x%x name: %s is_release: %d handled_by_ime: %d\n",
        ev->glfw_ev.native_key,
        glfw_xkb_keysym_name(ev->glfw_ev.key),
        is_release,
        handled_by_ime);
    if (window && !handled_by_ime && !(is_release && ev->glfw_ev.native_key == (int)prev_handled_press)) {
        debug(
            "↳ to application: glfw_keycode: 0x%x (%s) keysym: 0x%x (%s) action: %s %s text: %s\n",
            ev->glfw_ev.native_key,
            _glfwGetKeyName(ev->glfw_ev.native_key),
            ev->glfw_ev.key,
            glfw_xkb_keysym_name(ev->glfw_ev.key),
            (ev->glfw_ev.action == GLFW_RELEASE ? "RELEASE" : (ev->glfw_ev.action == GLFW_PRESS ? "PRESS" : "REPEAT")),
            format_mods(ev->glfw_ev.mods),
            ev->glfw_ev.text);

        ev->glfw_ev.ime_state = GLFW_IME_NONE;
        _glfwInputKeyboard(window, &ev->glfw_ev);
    } else debug("↳ discarded\n");
    if (!is_release && handled_by_ime) last_handled_press_keycode = ev->glfw_ev.native_key;
}
...
is_switch_layout_key(xkb_keysym_t xkb_sym) {
    return xkb_sym == XKB_KEY_ISO_First_Group || xkb_sym == XKB_KEY_ISO_Last_Group || xkb_sym == XKB_KEY_ISO_Next_Group || xkb_sym == XKB_KEY_ISO_Prev_Group ||
           xkb_sym == XKB_KEY_Mode_switch;
}
...
glfw_xkb_handle_key_event(_GLFWwindow *window, _GLFWXKBData *xkb, xkb_keycode_t xkb_keycode, int action) {
    ...
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        if (xkb_sym == syms[0]) { // composed sym is the same as non-composed sym
            ...
            if (consumed_unknown_mods) {
                debug("%s", format_xkb_mods(xkb, "consumed_unknown_mods", consumed_unknown_mods));
    ...
    if (is_switch_layout_key(xkb_sym)) {
        debug(" is a keyboard layout shift key, ignoring.\n");
        return;
    }
    ...
    debug(
        "%s%s: %d (%s) xkb_key: %d (%s)", format_mods(sg->modifiers), "glfw_key", glfw_sym, _glfwGetKeyName(glfw_sym), xkb_sym, glfw_xkb_keysym_name(xkb_sym));
    ...
    if (ibus_process_key(&ibus_ev, &xkb->ibus)) {
        debug(
            "↳ to IBUS: keycode: 0x%x keysym: 0x%x (%s) %s\n",
            ibus_ev.ibus_keycode,
            ibus_ev.ibus_keysym,
            glfw_xkb_keysym_name(ibus_ev.ibus_keysym),
            format_mods(ibus_ev.glfw_ev.mods));
    } else {
